...
pub async fn run(client: &McpClient, cmd: MembersCmd, json: bool) -> Result<()> {
    let value = match cmd {
        MembersCmd::Create {
            ...
            if let Some(v) = email { args.insert("email".into(), Value::String(v)); }
            if let Some(v) = phone { args.insert("phone".into(), Value::String(v)); }
            if let Some(v) = first_name { args.insert("firstName".into(), Value::String(v)); }
            if let Some(v) = last_name { args.insert("lastName".into(), Value::String(v)); }
            if enroll_membership { args.insert("enrollMembership".into(), Value::Bool(true)); }
            if let Some(v) = tier_id { args.insert("tierId".into(), Value::String(v)); }
            if let Some(v) = notes { args.insert("notes".into(), Value::String(v)); }
            client.call_tool("create_account", Value::Object(args)).await?
        }
